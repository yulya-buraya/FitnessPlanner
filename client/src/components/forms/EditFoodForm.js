...
export const EditFoodForm = ({ setEditForm, food, setFood }) => {
    ...
    const [form, setForm] = useState({
        name: food.name, category: food.category, proteins: food.proteins, fats: food.fats, carbhydrates: food.carbhydrates, calory: food.calory
    })
    ...
    const editFoodHandler = async () => {
        try {
            ...
            const product = await request(`/api/food/${food._id}`, 'PUT', { ...form })

            setFood(prev => {
                const products = [...prev];
                if (products.indexOf(food) >= 0) {
                    products.splice(products.indexOf(food), 1, form);
                }
                return products
            })
